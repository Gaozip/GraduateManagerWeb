<template>
	<el-form :inline="true" :model="form" ref="formName" :rules="rules">
		...
		<div class="table-wrap">
	      <el-table :data="tableData" border stripe style="width: 100%">
	        ...
	        <el-table-column  label="工作经验">
	        	<template slot-scope="wScope">
	        		<el-button type="text" @click="$refs.showWokeExperience.show(wScope.row)">查看</el-button>
	        	</template>
	        </el-table-column>
	        <el-table-column  label="项目经验">
	        	<template slot-scope="pScope">
	        		<el-button type="text" @click="$refs.showProjectExperience.show(pScope.row)">查看</el-button>
	        	</template>
	        </el-table-column>
	      ...
	      <editResume ref="editResume"></editResume>
		...
		// 修改每页显示条数
      	...
      	deleteResumeByResumeId(resumeId){
      		this.$confirm('确认删除？').then(_ => {
      			AXIOS.api('/resume/doDelete',{'resumeId':resumeId,}).then(data =>{
      	...
      	fetchTableData(){
      		...
      		RESUME_API.api(RESUME_API.URL_DO_SEARCH,param).then(data =>{
